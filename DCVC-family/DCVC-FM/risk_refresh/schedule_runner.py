...
P1 快照(协议 §5):
  ...
  --decode_mode full|suffix: 分支解码验证模式 (协议 11.2; suffix=从快照恢复 decoder 状态
      仅解码后缀, 5% 抽检用 full)

历史修复(全部经等价测试背书):
  shape-assert list 比较 / 快照目录覆盖 / 快照前 flush 缓冲区 / 分支源读取器帧对齐 /
  full 模式 branch_tag / 原生 SSE 字段 (协议 2.4) / suffix 解码 DPB+SPS 从快照恢复
整合版: 2026-09-18 (清除重复粘贴副本, 单一定义)
"""
...
def load_sps_state(snap_dir):
    with open(Path(snap_dir) / "sps_state.json") as f:
        state = json.load(f)
    helper = SPSHelper()
    helper.spss = [dict(s) for s in state["spss"]]
    return helper
...
def calc_distortion(x_hat, y, u, v, calc_ssim_flag):
    ...
    if calc_ssim_flag:
        ...
        ssim = 0.0
    sse_y = float(np.asarray(y - y_rec[0, :, :], dtype=np.float64).__pow__(2).sum())
    sse_u = float(np.asarray(u - uv_rec[0, :, :], dtype=np.float64).__pow__(2).sum())
    sse_v = float(np.asarray(v - uv_rec[1, :, :], dtype=np.float64).__pow__(2).sum())
    return psnr, psnr_y, psnr_u, psnr_v, ssim, sse_y, sse_u, sse_v
...
def load_snapshot(snap_dir, device, expect_schedule_id=None, expect_seq_id=None):
    ...
    helper = load_sps_state(snap_dir)
    ...
    with torch.no_grad():
        ...
        for t in range(start_idx, frame_num):
            ...
            if t in snap_frames and args.snapshot_dir:
                ...
                output_file.flush()  # 缓冲区落盘后再复制前缀, 否则 prefix.bin 截断
            ...
            psnr, psnr_y, psnr_u, psnr_v, msssim, sse_y, sse_u, sse_v = calc_distortion(
        ...
        # ─── 解码循环: 不读 schedule, 仅 NAL/SPS (E4) ───
        dec_helper = SPSHelper()
        input_file = bin_path.open("rb")
        src_reader = YUVReader(seq["src_path"], pic_w, pic_h)
        # P1.3 suffix 模式(协议 11.2): seek 到前缀末尾, decoder DPB 与 SPS 表
        # 一律从快照恢复(不能用编码循环的终态 dpb —— 那是第 frame_num-1 帧后的状态)
        if resume_meta and args.decode_mode == "suffix":
            input_file.seek(resume_meta["prefix_bytes"])
            for _ in range(start_idx):
                src_reader.read_one_frame(dst_format="420")
            _, dpb_dec, _ = load_snapshot(args.resume_snapshot, device,
                                          expect_seq_id=seq["sequence_id"])
            dec_helper = load_sps_state(args.resume_snapshot)
            decoded_n = start_idx
        else:
            decoded_n = 0
            dpb_dec = None
        ...
        while decoded_n < frame_num:
            ...
            assert s is not None, f"frame {decoded_n}: sps_id {sps_id} unknown (decoder SPS state incomplete)"
            if new_stream:
...
def parse_args():
    ...
    ap.add_argument("--decode_mode", default="full", choices=["full", "suffix"])
